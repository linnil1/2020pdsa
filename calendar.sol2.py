...
class Node:
    ...
    def update(self):
        ...
        if self.left is not None:
            ...
            if self.left.color == NodeColor.RED:
                self.level = self.left.level
            else:
                self.level = self.left.level + 1
...
class RBTree:
    ...
    def insBalance(self, r):
        """Balance the tree after Insert"""
        if not self.isRed(r.left) and self.isRed(r.right):
            r = self.rotateLeft(r)
        if self.isRed(r.left) and self.isRed(r.left.left):
            r = self.rotateRight(r)
        if self.isRed(r.left) and self.isRed(r.right):
            r = self.flipColors(r)
        r.update()
        return r
